...
func Test_dashboardImport(t *testing.T) {
	var (
		root string
		err  error
	)
	if root, err = utils.PkgRootPath(UTEmptyStruct{}); err != nil {
		t.Errorf("dashboardImport() error = %v", err)
	}
	type args struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := dashboardImport(tt.args.o); (err != nil) != tt.wantErr {
				t.Errorf("dashboardImport() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
